...
// has no dependency on CyberDrain's internal hosting/management plane ΓÇö the deployed instance
...
//
// NOTE: the web app name and the key vault name MUST be identical ΓÇö the backend resolves
...
// blob access, infrastructure (double) encryption ΓÇö settable only at creation time.
...
// Azure Files ΓÇö maximum-security SMB posture. The containerized app does not use file
...
// Key Vault Secrets ΓÇö created with placeholder values by default so a fresh instance
